<template>
	<div v-if="participantId" v-show="isVisible" class="participant-details">
		<!-- header section -->
		<div class="participant-details__header">
			<h2 class="participant-details__name">{{ name }}</h2>
			<span
			...
			<div @click="close" class="participant-details__close">
		...
		<div class="participant-actions">
			<div class="vds-button-group">
				<button id="message-popover-trigger" class="vds-button-group__button">
					<i class="vds-icon vds-icon--blue vds-icon--email"></i>
					Bericht
				</button>

				<button class="vds-button-group__button">
					<i class="vds-icon vds-icon--blue vds-icon--textsms"></i>
					Notitie
				</button>

				<button id="status-popover-trigger" class="vds-button-group__button">
					<i class="vds-icon vds-icon--blue vds-icon--textsms"></i>
					Status
				</button>

				<button id="remove-popover-trigger" class="vds-button-group__button">
					<i class="vds-icon vds-icon--red vds-icon--delete"></i>
				</button>
			</div>
		</div>

		<AppPopover name="message-popover">
			<template #title>Een bericht naar {{participant.participant.voornaam}} sturen</template>
			...
			<template #content>
				...
				>Nieuw bericht</div>
				<div class="message-option" v-else>Antwoorden op bericht</div>
				<div v-if="!participant.status_bevestigd" class="message-option">Bevestiging sturen</div>
			</template>
		</AppPopover>

		<AppPopover name="status-popover">
			<template #title>{{participant.participant.voornaam}}'s status updaten</template>

			<template #content>
				<div class="vds-control statusses">
					...
					<label v-if="startsInLessThanAnHour" class="vds-label vds-label--required">
						<input
							type="checkbox"
							class="vds-checkbox vds-checkbox--large"
		...
		<AppPopover name="remove-popover">
			<template #title>{{participant.participant.voornaam}} uit de planning halen</template>
			...
			<template #content>
				<form class="remove-form">
					...
					<div>
						<input id="unsuitable-input" type="checkbox" name="remove" />
						<label
							class="remove-form__choice"
					...
					<div>
						<input id="noshow-input" type="checkbox" name="remove" :checked="participant.status_noshow" />
						<label
							class="remove-form__choice"
							for="noshow-input"
		...
		<ParticipantNotes :participantId="participant.id"></ParticipantNotes>
		<!-- tabs section -->
		<div class="tabs-section">
			...
			<div v-show="tabs[0].active" class="participant-details__tab profile-tab">
				<ParticipantNotes v-if="participant.konijn_id" :konijnId="participant.konijn_id"></ParticipantNotes>
				<div
					v-if="participant.participant.geslacht || participant.participant.age"
					class="profile-tab__block"
				>
					<div class="profile-tab__block__title">Geslacht & Leeftijd</div>
					<div class="profile-tab__block__content">
						{{sexHuman}},
						{{ageHuman}}
					</div>
				</div>
				<div v-if="participant.participant.opleiding" class="profile-tab__block">
					<div class="profile-tab__block__title">Opleiding</div>
					<div class="profile-tab__block__content">{{participant.participant.opleiding}}</div>
				</div>
				<div
					v-if="participant.participant.city || participant.participant.woonplaats"
					class="profile-tab__block"
				>
					<div class="profile-tab__block__title">Woonplaats</div>
					<div v-if="participant.participant.city" class="profile-tab__block__content">
					...
					>{{participant.participant.woonplaats}}</div>
				</div>

				<a class="wide">Bekijk respondent pagina</a>
				<div class="vds-line vds-line--small wide"></div>

				<div class="profile-tab__block wide">
					<div class="profile-tab__block__title">Profiel</div>
				...
				<div class="profile-tab__block wide">
					<div class="profile-tab__block__title">Achtergrond</div>
					...
					>
						<template #trigger>
							<ul>
								<li v-for="item of backgroundItems" :key="item.index">{{item}}</li>
							</ul>
						</template>
					</AppInlineEdit>
				</div>
			...
			<!-- messages tab -->
			<div v-show="tabs[1].active" class="participant-details__tab">
				<div v-if="!participant.messages || !participant.messages.length">Nog geen berichten</div>
				<ParticipantMessages :participantId="participantId"></ParticipantMessages>
			</div>

			<!-- compensation tab -->
			<div v-show="tabs[2].active" class="participant-details__tab">
				<div class="vds-control">
					...
					/>
					<div class="vds-control__help">Std vergoeding voor dit project: €{{ projectCompensation}}</div>
				...
				<div class="vds-line vds-line--small wide"></div>
				<ul v-if="participant.consent && participant.consent.id" class="consent-details">
					<li>Bevestigd en akkoord gegaan met toestemmingsverklaring.</li>

					<li v-if="participant.consent.iban">Bankgegevens ingevuld {{ participant.consent.iban }}</li>

					<li
						v-if="participant.consent.payable_amount"
					>Incentive + reiskosten berekend: totaal € {{participant.consent.payable_amount }}</li>

					<li
						v-if="participant.consent.paid_on"
					>Overgemaakt op {{ timeHuman(participant.consent.paid_on) }}</li>

					<li
						v-else-if="participant.consent.iban && !participant.consent.payment_status !== 99"
					>Nog niet overgemaakt</li>
					<li>
						<a
							target="_parent"
