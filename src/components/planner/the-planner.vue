<template>
	<div class="planner">
		<div class="planner-header">
			<h1 class="planner-header__title">Planning</h1>
			<button
			...
			>{{unscheduledParticipants.length ? "Respondent toevoegen" : "Alle respondenten zijn ingepland"}}</button>
		</div>

		<TheSchedule :scheduleItemsListVisible="scheduleItemsListVisible">
			<template #scheduleItems>
				<ScheduleItem
					...
					:confirmText="`${participant.participant.name ? participant.participant.name : participant.participant.voornaam} heeft al bevestigd. Als je doorgaat, moet je ${participant.participant.name ? participant.participant.name : participant.participant.voornaam} de nieuwe tijd wel doorgeven. Wil je doorgaan?`"
				...
				>
					<ParticipantThumbnail :participantId="participant.id" />
				</ScheduleItem>
			</template>
